    ...
    class Badge implements ControllerProviderInterface {
        ...
        public function connect(Application $app) {
            ...
            $controller->get("/{id}/", array($this, 'index'))
                ->assert('id', '\d+')->bind('backend-badge');
            
            $controller->get("/{id}/criteria", array($this, 'criteria'))
                ->assert('id', '\d+')->bind('backend-badge-criteria');
        ...
        public function index(Application $app, $id) {
            ...
            return $app->json(array(
                    ...
                    'issuer' => $app['url_generator']->generate(
                        'backend-issuer', 
                        array(), 
                        UrlGenerator::ABSOLUTE_URL
                    ),
                    'criteria' => $app['url_generator']->generate(
                        'backend-badge-criteria', 
                        array('id' => $id), 
                        UrlGenerator::ABSOLUTE_URL
                    ),
            ));
        ...
        private function getBadge(Application $app, $id) {
            ...
            $badge = $app['orm.em']->getRepository('Model\Badge')
                ->findOneById($id);
